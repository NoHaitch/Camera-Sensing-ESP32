...
static void publish_image_once(esp_mqtt_client_handle_t client, int attempt_index)
{
    const uint8_t *image_data = test_image_jpg_start;
    size_t image_size_bytes = (size_t)(test_image_jpg_end - test_image_jpg_start);
    ...
    int raw_msg_id = esp_mqtt_client_publish(
        client,
        raw_topic,
...
{
    const size_t image_size_bytes = (size_t)(test_image_jpg_end - test_image_jpg_start);
    ...
    if (strcmp(MODE, "experiment") == 0) {
        for (int attempt_index = 1; attempt_index <= N_SEND; attempt_index++) {
            ...
            publish_image_once(g_client, attempt_index);
